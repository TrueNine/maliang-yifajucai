create
    table
        if not exists job_seeker(
            ...
            ex_min_salary decimal(
                10,
                2
            ) not null,
            ex_max_salary decimal(
                10,
                2
            ),
...
select
    add_base_struct('work_experience');
...
select
    add_presort_tree_struct('industry');
...
select
    add_presort_tree_struct('enterprise');

select
    add_base_struct('enterprise');
...
select
    add_base_struct('job_rule_address');
