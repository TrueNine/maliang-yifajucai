...
import type {
  AuthApi_AccountDto, 
  IPage, 
  SaTokenService_SaTokenLoginView, 
...
export class AuthApi {
  ...
  > = async(options) => {
    ...
    return (await this.executor({uri: _uri, method: 'POST', body: options.body})) as Promise<SaTokenService_SaTokenLoginView>;
  }
  ...
  readonly loginByWxpaCode: (options: AuthApiOptions['loginByWxpaCode']) => Promise<
    SaTokenService_SaTokenLoginView | undefined
  > = async(options) => {
    ...
    _uri += encodeURIComponent(_value);
    _separator = '&';
    return (await this.executor({uri: _uri, method: 'POST'})) as Promise<SaTokenService_SaTokenLoginView | undefined>;
  }
